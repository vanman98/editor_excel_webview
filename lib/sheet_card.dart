...
class SheetChangesCard extends StatefulWidget {
  ...
  /// sheet ops (optional)
  final List<Map<String, dynamic>> sheetOps;
...
class SheetChangesCardState extends State<SheetChangesCard> {
  ...
  Widget build(BuildContext context) {
    final totalCells = widget.items.length;
    final totalOps = widget.sheetOps.length;
          ...
          if (_expanded) ...[
            ...
            if (widget.sheetOps.isNotEmpty) ...[
              ...
              Padding(
                ...
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: widget.sheetOps.take(20).map((op) {
                    final ts = widget.fmtTs(op['ts'] as int?);
                    final t = (op['op'] ?? '').toString();

                    // renamed / reordered may have from/to
                    final from = op['from'];
                    final to = op['to'];
                    final extra = (from != null || to != null)
                        ? ' ($from → $to)'
                        : '';
            ...
            if (widget.items.isEmpty)
              const Padding(
                ...
                child: Text(
                  'No cell changes on this sheet yet.',
